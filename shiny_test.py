...
def is_reactive_value(value):
    return hasattr(value, '_reactive') or hasattr(value, '_value')

def get_value(value):
    """Helper function to handle both reactive and non-reactive values"""
    if hasattr(value, '_reactive'):
        return value()
...
def image_gallery(
    ...
    # Get actual values from reactive values
    images_value = get_value(images)
    label_value = get_value(label)
    image_labels_value = get_value(image_labels)
    image_size_value = get_value(image_size)
    initial_selected_indices_value = get_value(initial_selected_indices)

    if images_value is None or len(images_value) == 0:
        return None

    import numpy as np
    from PIL import Image
    from util import encode_numpy, encode_PIL_Image

    images_final = []
    for i, image in enumerate(images_value):
        if isinstance(image, str):
            ...
            raise ValueError(
                f"Image must be an image file, a PIL Image, or a 2D numpy array. You have provided {type(image)}"
            )
        images_final.append(tmp)

    assert len(image_labels_value) == 0 or len(image_labels_value) == len(images_final)

    if len(image_labels_value):
        image_labels_final = image_labels_value
    else:
        image_labels_final = list(range(1, len(images_final) + 1))

    assert image_size_value >= 32
    ...
    def create_image_button(
        ...
        img = ui.img(
            ...
            alt=f"Image {i+1}",
            title=str(label),
            style=f"object-fit: contain; height: {image_size_value}px; border: {image_border}px solid transparent;",
        )
    ...
    if enable_selection and len(initial_selected_indices_value) > 0:
        click_scripts = []
        for i in initial_selected_indices_value:
