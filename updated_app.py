...
local_emd_file_path = "C:\\Anika's Folder\\Random\\emd_10499.map"
...
def server(input, output, session):
    ...
    @render_plotly
    @reactive.event(data_all, input.negate, apix_reactive)
    def transformed_display_micrograph():
        images = data_all() # selected_images()
        if data_all() is None:
            return None # px.scatter(title="No image selected")

        # h, w = selected_images()[0].shape[:2]
        h, w = data_all().shape[:2]
        nx.set(h)
        ny.set(w)
